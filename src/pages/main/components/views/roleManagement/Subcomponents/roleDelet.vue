<template>
    <div class="Modify">
        <el-dialog
            ...
            <!--            输入框-->
            <div>
                你确定要删除角色<span style="color:orangered;font-weight: bold;font-size: 20px">{{userInfo.name}}</span>嘛？
            </div>
...
<script>
    import { deletRole} from '@/network/main'
    export default {
        name: "roleDelet",
        ...
        methods:{
            ...
            //发送删除角色id的请求
            deletRole(params){
                deletRole('/role',params,'delete').then(res=>{
                    console.log(res)
                    if(res.msg == '成功'){
                        this.$message({
                            message: '删除成功',
